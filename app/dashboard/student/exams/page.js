...
useEffect(() => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        for (const exam of data) {
          try {
            const submissionResponse = await fetch(`http://localhost:3001/api/exams/${exam._id}/submissions`, {
              headers: { 'Authorization': `Bearer ${token}` }
            });
            if (submissionResponse.ok) {
              const submissionData = await submissionResponse.json();
              const userSubmission = submissionData.find(sub => sub.student._id === JSON.parse(localStorage.getItem('user')).id);
              if (userSubmission) {
  ...
  const startExam = async (exam) => {
    try {
      ...
      setMessage({ type: 'error', content: 'Failed to start exam in fullscreen mode' });
    }
  };
  ...
  const autoSaveProgress = async () => {
    ...
    try {
      ...
      await fetch(`http://localhost:3001/api/exams/${selectedExam._id}/auto-save`, {
        method: 'PUT',
        headers: {
  ...
  const submitExam = async (isAutoSubmit = false) => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:3001/api/exams/${selectedExam._id}/submit`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
      ...
      if (response.ok) {
        ...
        setMessage({ 
          type: 'success', 
          content: isAutoSubmit ? 'Exam auto-submitted due to time limit' : 'Exam submitted successfully!' 
        });
        ...
        setMessage({ type: 'error', content: data.message || 'Failed to submit exam' });
      }
    } catch (error) {
  ...
  const getExamStatus = (exam) => {
    ...
    if (submission) {
      if (submission.status === 'graded') {
        return { status: 'graded', color: 'bg-green-100 text-green-800', text: 'Completed' };
      }
      return { status: 'submitted', color: 'bg-blue-100 text-blue-800', text: 'Submitted' };
    }
      ...
      <div className="min-h-screen bg-white p-8">
        ...
        <div className="flex items-center justify-between mb-8 pb-4 border-b border-gray-200">
          <div>
            ...
            <p className="text-gray-600">{selectedExam.subject} • {selectedExam.totalMarks} marks</p>
          </div>
            ...
            <div key={question._id} className="bg-gray-50 p-6 rounded-lg">
              <div className="mb-4">
                <h3 className="text-lg font-semibold text-gray-900 mb-2">
                  Question {index + 1} ({question.points} mark{question.points > 1 ? 's' : ''})
                </h3>
        ...
        <div className="fixed bottom-4 left-4 bg-yellow-100 border border-yellow-400 text-yellow-800 px-4 py-2 rounded-lg">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm">Do not switch tabs or exit fullscreen during the exam</span>
          </div>
        </div>
      </div>
    );
          ...
          <div className="flex-1 flex flex-col lg:ml-0">
            <main className="flex-1 p-6 lg:p-8">
              ...
              <div className="mb-8">
                ...
                <p className="text-gray-600">Take exams assigned by your teachers</p>
              </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
                {filteredExams.map((exam) => {
                  ...
                  return (
                    <div key={exam._id} className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                      ...
                      <div className="space-y-3">
                          ...
                          <div className="bg-green-50 p-3 rounded-lg">
                            <div className="flex items-center justify-between">
                              ...
                              <span className="text-lg font-bold text-green-600">
                                {submission.totalScore}/{exam.totalMarks} ({submission.percentage.toFixed(1)}%)
                              </span>
                            </div>
                          </div>
                        )}
                        ...
                        {status.status === 'upcoming' && (
                          <div className="text-center py-2">
                            ...
                            <p className="text-sm text-yellow-600">Available from</p>
                        ...
                        {status.status === 'submitted' && (
                          <div className="text-center py-2">
                            ...
                            <p className="text-sm text-green-600">Exam completed</p>
                            <p className="text-xs text-gray-500">
                        ...
                        {status.status === 'expired' && (
                          <div className="text-center py-2">
                            ...
                            <p className="text-sm text-red-600">Exam expired</p>
                          </div>
                        )}
                      </div>
                    </div>
              ...
              {filteredExams.length === 0 && (
                <div className="text-center py-12">
                  ...
                  <p className="text-gray-600">
                    {searchTerm ? 'No exams match your search criteria' : 'No exams have been assigned to you yet'}
                  </p>
                </div>
              )}
            </main>
