...
export default function TeacherStudents() {
  ...
  const fetchStudents = async () => {
    try {
      ...
      // Fetch my students
      const myStudentsResponse = await fetch(
        "http://localhost:3001/api/users/my-students",
        {
      ...
      const availableResponse = await fetch(
        "http://localhost:3001/api/users/available-students",
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      );
  ...
  const assignStudent = async (studentId) => {
    ...
    try {
      ...
      const response = await fetch(
        "http://localhost:3001/api/users/assign-student",
        {
          method: "POST",
          headers: {
      ...
      if (response.ok) {
        setMessage({
          type: "success",
          content: "Student added to your class successfully!",
        });
        ...
        setMessage({
          type: "error",
          content: data.message || "Failed to assign student",
        });
      }
    } catch (error) {
      setMessage({
  ...
  const removeStudent = async (studentId) => {
    ...
    try {
      const token = localStorage.getItem("token");
      const response = await fetch(
        "http://localhost:3001/api/users/remove-student",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      if (response.ok) {
        setMessage({
          type: "success",
          content: "Student removed from your class successfully!",
        });
        fetchStudents();
      } else {
        setMessage({
          type: "error",
          content: data.message || "Failed to remove student",
        });
      }
    } catch (error) {
      setMessage({
                  ...
                  <div className="text-center py-12">
                    ...
                    <p className="text-gray-600">
                      {searchTerm
                        ...
                        : "Add students to start teaching"}
                    </p>
                  </div>
                )}
              </main>
