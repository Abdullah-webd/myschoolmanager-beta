...
export default function TeacherAI() {
  ...
  const sendMessage = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (!currentSession) {
          setCurrentSession({ sessionId: data.sessionId, title: userMessage.substring(0, 50) });
          fetchSessions(); // Refresh sessions list
        }
      } else {
        setMessage({ type: 'error', content: data.message || 'Failed to send message' });
      }
    } catch (error) {
  ...
  const deleteSession = async (sessionId) => {
    ...
    try {
      ...
      if (response.ok) {
        setMessage({ type: 'success', content: 'Chat session deleted successfully' });
          ...
          <div className="flex-1 flex">
            ...
            <div className="w-80 bg-white border-r border-gray-200 flex flex-col">
              <div className="p-6 border-b border-gray-200">
                ...
                <p className="text-sm text-gray-600">Ask questions about teaching, curriculum, or get help with exam creation.</p>
              </div>
                ...
                {sessions.map((session) => (
                  <div
                    ...
                    className={`p-4 border-b border-gray-200 cursor-pointer hover:bg-gray-50 transition-colors ${
                      currentSession?.sessionId === session.sessionId ? 'bg-green-50 border-l-4 border-l-green-500' : ''
                    }`}
                  >
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-600">
                      Ask me anything about teaching, curriculum, or exam creation
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="flex-1 overflow-y-auto p-6 bg-gray-50">
                {messages.length === 0 ? (
                  <div className="text-center py-12">
                    ...
                    <p className="text-gray-600 mb-6">Ask me about teaching strategies, curriculum planning, or exam creation.</p>
