// Public route for teachers/students to check their school's subscription status
...
const FLUTTERWAVE_SECRET_KEY =
  "FLWSECK_TEST-769e9db540aef513a9973427f66f4daf-X";
...
router.post("/initialize", auth, async (req, res) => {
  try {
    ...
    if (!school) {
      return res
        .status(404)
        .json({ message: "School not found for this admin" });
    }
...
router.get("/school-subscription", async (req, res) => {
  try {
    ...
    // Accept either adminId or schoolId for flexibility
    const query = adminId
      ? { admin: adminId }
      : schoolId
      ? { _id: schoolId }
    ...
    if (!query) {
      return res
        .status(400)
        .json({ message: "adminId or schoolId is required" });
    }
    const school = await require("../models/School").findOne(query);
    if (!school) {
      return res.status(404).json({ message: "School not found" });
    }
    school.checkSubscriptionStatus();
    await school.save();
    res.json({ subscription: school.subscription });
  } catch (error) {
    console.error("School subscription check error:", error);
    res.status(500).json({ message: "Server error" });
  }
});
